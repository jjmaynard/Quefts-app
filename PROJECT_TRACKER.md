...
**Addendum, 2026-09-13 — two major pre-existing bugs found and fixed in `spatial_data_integration.R`, discovered when the user tried the running app against the real (non-mocked) API and got "Could not retrieve soil data for this location":**

Every prior phase's testing (Phases 4, 6, 7) had mocked `fetch_soilgrids_data()` specifically to avoid the live SoilGrids API's documented flakiness (`PROJECT_EVALUATION.md` §6.3). Investigating the user's live failure properly (reproducing the exact HTTP request with `curl`, bisecting which parameter caused it, then testing against a real running R session) found that **the "flakiness" was never real API unreliability — it was two bugs in this project's own request/response handling that meant a live, successful spatial-data fetch had never actually worked, in this project's entire history:**

1. **`fetch_soilgrids_data()`'s query was malformed.** It joined multi-valued query parameters with commas (`property = paste(properties, collapse = ",")`, similarly for `depth`), producing e.g. `?property=phh2o,soc,nitrogen,...`. Confirmed by direct `curl` testing that SoilGrids' actual API returns a reproducible `500` for *any* request with two or more comma-joined properties — even `?property=phh2o,soc` alone — while every single-property request succeeds. The API requires the parameter **repeated** once per value (`?property=phh2o&property=soc&...`), not comma-joined. Fixed by building `query_params` with `setNames(as.list(properties), rep("property", length(properties)))` (and the same pattern for `depth` and `value`) so `httr::GET()` emits repeated keys.
2. **`extract_soilgrids_properties()` could never have parsed a successful response, either.** It indexed `data$properties$layers[[prop]]` as if `layers` were a list keyed by property name. In reality `jsonlite::fromJSON()` parses the API's `"layers"` JSON array into a **data.frame** (one row per property, with `name` as a column and `depths` as a list-column of per-depth data.frames) — indexing that data.frame with `[["phh2o"]]` always returns `NULL`. Even on a 200 response, this function silently extracted nothing, so every field ended up coming from `estimate_missing_quefts_params()`'s defaults instead of real SoilGrids values. Fixed to filter the data.frame by `layers$name == prop` and read `prop_row$depths[[1]]` (itself a data.frame, filtered by `label == depth`) for the actual mean/uncertainty values.

**Verification:** direct `curl` reproduction of both the broken request (500, reproducible on demand) and the fixed one (200, real data) against the live SoilGrids API; a live (non-mocked) call to `get_multiscale_soil_data(lat=7.5, lon=-1.5, region="Sub-Saharan_Africa")` now returns real fetched values (e.g. `pH=6.1`, `OC=37.7`) instead of `NULL`; a full, non-mocked `comprehensive_fertilizer_recommendation()` run completes with `report_generation_note: NULL` and a real decision; re-ran the full `testthat` suite (still passing — it uses a mock specifically so it doesn't depend on the live API, unaffected by this fix); and re-tested the exact request from the user's screenshot (`lat=2.3, lon=20.5`) against the running API, which now returns `200` with a real recommendation instead of the "Could not retrieve soil data" error. This means the core spatial-data-integration feature this entire project is built around is, as far as this history shows, working end-to-end with live data for the first time.
...
#### Result / Implementation Notes
**2026-09-13** — User's chosen targets: **Railway** for the API, **Vercel** for the frontend. Left unchecked because nothing is actually deployed yet — I have no Railway/Vercel accounts or credentials to do that myself, and actual deployment (creating live, possibly billed, publicly-reachable infrastructure) isn't something to do without the account holder driving it. What's done is everything that can be prepared without those credentials:

- **`api/Dockerfile`** — `rocker/geospatial:4.5.2` base (matches the R version pinned in `renv.lock`, and already ships GDAL/GEOS/PROJ compiled for `sf`/`terra`/`raster`, avoiding a separate system-library install step), `renv::restore()` for reproducible dependency install, then copies in `R/`, `data/`, `api/` only (not `web/`, `dev-history/`, `docs/`, `experimental/`, `tests/`, `demo/` — none of which the running API needs). **Confirmed the base image tag actually exists** (queried Docker Hub's API directly) since I can't `docker build` it locally to check any other way — no Docker install in this environment. Everything past "does this base image exist" (does `renv::restore()` actually succeed on it, does the container actually start) is unverified.
- **`.dockerignore`** (repo root, since the build context must be root for the Dockerfile's `COPY R/`/`COPY data/`/`COPY api/` to resolve) and **`railway.json`** (points Railway at `api/Dockerfile` with root context, plus a `/health` healthcheck using the endpoint already built in Phase 6).
- **`api/run_api.R`**: added `HOST` env var (defaults to `127.0.0.1`, unchanged local-dev behavior; the Dockerfile sets it to `0.0.0.0`, required for Railway's proxy to reach the container — its own loopback interface isn't externally reachable). `PORT` already read an env var from Phase 6, which is what Railway needs.
- **`api/plumber.R`**: the CORS filter's `Access-Control-Allow-Origin` was hardcoded to `"*"` since Phase 7 (fine for local dev, explicitly flagged then as needing fixing before a real deployment). Now reads `ALLOWED_ORIGIN` (env var, defaults to `"*"` so local dev is unaffected) so it can be locked to the actual Vercel URL in production.
- **`DEPLOYMENT.md`** — step-by-step for both Railway and Vercel, including the specific env vars each needs from the other (`ALLOWED_ORIGIN` on Railway needs the Vercel URL; `NEXT_PUBLIC_API_URL` on Vercel needs the Railway URL, so there's an unavoidable two-pass setup: deploy the API first, then the frontend, then circle back to set `ALLOWED_ORIGIN` and redeploy the API) — plus two known limitations documented rather than fixed, to keep this phase's scope to "containerize + deploy" rather than a persistence/CORS redesign:
  - Railway's filesystem is ephemeral, so Phase 8's file-based `/results/[id]` persistence doesn't survive a redeploy (works fine across requests within one running deployment). A Railway Volume or a real database would fix this; neither is implemented.
  - `ALLOWED_ORIGIN` is a single origin string, so it won't also allow Vercel's per-branch preview-deployment URLs — fine for a production-only setup, a real gap if preview deployments need to call the live API.
- No frontend Dockerfile — Vercel builds Next.js natively from the repo, no container needed on that side. Documented setting **Root Directory = `web`** in Vercel's project settings, since this is a monorepo.

**Verified without Docker or any deployment:** the actual behavior of every code change, run locally. `Rscript api/run_api.R` with `HOST`/`PORT`/`ALLOWED_ORIGIN` all unset still binds `127.0.0.1:8000` with `Access-Control-Allow-Origin: *` (default local-dev behavior unchanged); the same command with `HOST=0.0.0.0`, `PORT=8001`, `ALLOWED_ORIGIN=https://example-app.vercel.app` set correctly binds the new port on all interfaces and returns the configured origin in the CORS header. Full `parse()` syntax check across all 47 R files, and the full `testthat` suite still passing.

**What's genuinely left, and has to be done by the account holder, not me:** create the Railway and Vercel projects per `DEPLOYMENT.md`, watch the first Docker build's actual output (most likely failure point: `renv::restore()` not finding a binary for `sf`/`terra`/`raster`/`Rquefts` for this exact image and falling back to a slow source compile, or failing outright), and set the `ALLOWED_ORIGIN` / `NEXT_PUBLIC_API_URL` env vars on each side once the other's URL is known.
